...
public class JoystickController : MonoBehaviour
{
    ...
    private float size = 120; // size(width and height) of joystick element, modify it if you want
    private float sensitivity = 100; // the higher, the more sensitive. 0 means sudden switches between directions(no sensitivity)
    ...
    private void UpdateJoystick(PointerMoveEvent _ev)
    {
        if (detectJoystickMovement)
        {
            ...
            joystickKnob.style.translate = new StyleTranslate(new Translate(new Length(input.x * size / 2, LengthUnit.Pixel), new Length(-input.y * size / 2, LengthUnit.Pixel), 0f));
        }
    }
    ...
    private void HideJoystick(PointerUpEvent _ev)
    {
        ...
        joystickKnob.style.translate = new StyleTranslate(new Translate(new Length(0, LengthUnit.Pixel), new Length(0, LengthUnit.Pixel), 0f));
    }
